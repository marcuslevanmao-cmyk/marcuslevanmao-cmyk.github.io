...
const EditorEngine = (() => {
  ...
  function switchTab(id) {
    ...
    if (currentTab && activePage) {
      // Save the combined content of all pages
      const pages = document.querySelectorAll('.doc-page');
      let fullHtml = '';
      pages.forEach(p => fullHtml += p.innerHTML);
      currentTab.htmlContent = fullHtml;
    }
  ...
  function createNewTab() {
    ...
    const newTab = {
      ...
      htmlContent: `<div><p>Start writing here...</p></div>`
    };
  ...
  // ----- Pagination logic -----
  function splitContentIntoPages(html, pageHeight) {
    // We'll split on paragraph boundaries (<p> or <br>)
    // Create a temporary div to measure
    const temp = document.createElement('div');
    temp.style.cssText = 'position:absolute; visibility:hidden; width:816px; padding:96px; font-family: "Times New Roman", Times, serif; font-size:16px; line-height:1.6;';
    ...
    // Get all top-level nodes (paragraphs, headings, lists, etc.)
    const children = Array.from(temp.childNodes);
    for (let node of children) {
      // Clone node for measuring
      const testDiv = document.createElement('div');
      testDiv.style.cssText = temp.style.cssText;
      testDiv.innerHTML = currentPageHtml + node.outerHTML;
      document.body.appendChild(testDiv);
      const height = testDiv.scrollHeight;
      document.body.removeChild(testDiv);

      if (height > pageHeight && currentPageHtml !== '') {
        // Current page is full, push it and start new
  ...
  function renderPages(htmlContent) {
    ...
    // Define page height (1056px is the min-height, but we want to split when overflow)
    const pageHeight = 1056; // approximate
    const pagesHtml = splitContentIntoPages(htmlContent, pageHeight);

    pagesHtml.forEach((content, index) => {
      ...
      page.addEventListener('input', () => {
        // When user types, check if this page overflows
        checkAndPaginate(page);
        // Save content after a delay
  ...
  function checkAndPaginate(page) {
    ...
    if (page.scrollHeight > pageHeight + 50) { // overflow
      // Split this page's content
      const container = page.parentElement;
      const allPages = container.querySelectorAll('.doc-page');
      const idx = Array.from(allPages).indexOf(page);
      if (idx === -1) return;

      // Get the current page's HTML
      const html = page.innerHTML;
      // Split at the last paragraph that fits
      const temp = document.createElement('div');
      temp.style.cssText = 'position:absolute; visibility:hidden; width:816px; padding:96px; font-family: "Times New Roman", Times, serif; font-size:16px; line-height:1.6;';
      document.body.appendChild(temp);
      let splitIndex = 0;
      const children = Array.from(temp.childNodes);
      // We'll split by paragraph tags
      const parts = html.split(/(<p[^>]*>.*?<\/p>)/i); // simple split on p tags
      let currentHeight = 0;
      let firstPart = '';
      let secondPart = '';
      for (let part of parts) {
        temp.innerHTML = firstPart + part;
      ...
      if (secondPart.trim() === '') return; // no overflow

      // Update current page content to firstPart
      ...
      newPage.dataset.pageIndex = idx + 1;

      // Insert after the current page (and its page number)
      const pageNum = page.nextElementSibling; // .page-number
      if (pageNum) {
        container.insertBefore(newPage, pageNum.nextSibling);
      } else {
        container.appendChild(newPage);
      }

      // Update page numbers
      const allPageNums = container.querySelectorAll('.page-number');
      allPageNums.forEach((num, i) => {
        num.textContent = i + 1;
      });

      // Reattach event listeners to new page
      newPage.addEventListener('input', () => {
        checkAndPaginate(newPage);
        HistoryEngine.scheduleSnapshot('Auto-saved layout edit');
      });

      // Also, we need to update the tab's content
      // We'll do that on save, but we can also update periodically
    }
  }
  ...
  function loadActiveTabContent() {
    ...
    // Render pages from the tab's htmlContent
  ...
  // Helper to get combined HTML of all pages
  ...
  // Save the combined HTML to the current tab
